...
contains
  ...
  function advanceTimeMrtSolverBoussinesq2D( this ) result(res)
    use omp_lib
    class(MrtSolverBoussinesq2D), intent(inout) :: this
    ! local vars
    integer(IK) :: x, y, i, old, new, res
    integer(IK), dimension(0:1) :: dest
    real(RK) :: fluidMoms(0:8), tempMoms(0:4), &
      fluidEqMoms(0:8), tempEqMoms(0:4)
    integer, save :: numThreads = -9999
    ...
    !$omp parallel &
    !$omp shared(this, old, new, numThreads) private(x, y, i) &
    !$omp firstprivate(fluidMoms, tempMoms, fluidEqMoms, tempEqMoms, dest)

    !$omp single
    if( numThreads == -9999 ) then
    ...
    res = numThreads
  end function advanceTimeMrtSolverBoussinesq2D
